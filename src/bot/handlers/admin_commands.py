...
async def cmd_health(message: Message, container: Container) -> None:
    ...
    try:
        ...
        redis_time = (time.perf_counter() - t0) * 1000
        redis_status = "✅"
    except Exception as e:
        redis_status = f"❌ ({str(e)})"
    ...
    msg = (
        ...
        f"🔴 <b>Redis:</b> {redis_status} <code>{redis_time:.2f}ms</code>\n"
...
async def cmd_admin_chats(message: Message, container: Container):
    ...
    # Получаем ID чатов из Redis (через сервис или напрямую)
    # Используем приватный метод сервиса или доступ к redis напрямую
    active_chats_key = "active_chats"
    redis = container.chat_activity_service.redis
    chat_ids = await redis.smembers(active_chats_key)
    ...
    for chat_id_bytes in chat_ids:
        ...
        try:
            ...
            if "kicked" in err_str or "chat not found" in err_str:
                ...
                await redis.srem(active_chats_key, chat_id_bytes)
...
async def cmd_admin_help(message: Message) -> None:
    ...
    username = message.from_user.username
    mention = f"@{username}" if username else f"<b>{html.escape(message.from_user.full_name)}</b>"
    await message.answer(f"{mention}\n\n{get_admin_help()}", parse_mode="HTML")
