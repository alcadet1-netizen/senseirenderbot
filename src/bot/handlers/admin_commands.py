...
def get_admin_help() -> str:
    ...
    lines = [
        ...
        Visuals.frame_line_left("���🛡��️ АДМИН-ПАНЕЛ�Ь v2.3", w, "center"),
        ...
        Visuals.frame_separator_left(w),
        Visuals.frame_line_left("» СОЗ�ЫВ И РАЗДАЧА", w),
        ...
        Visuals.frame_separator_left(w),
        Visuals.frame_line_left("» НАГРАД�Ы", w),
...
async def cmd_cancel(message: Message, state: FSMContext):
    ...
    if current_state is None:
        await message.answer("�ℹ��️ Нет активных действий для отмены.")
        return
...
async def cmd_admin_chats(message: Message, container: Container):
    ...
    # Формируем отчет
    text = f"���📂 <b>АКТИВН�ЫЕ ГРУПП�Ы ({len(active_chats)})</b>\n\n"
