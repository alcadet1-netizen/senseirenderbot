...
async def cmd_profile(message: Message, container: Container) -> None:
    ...
    # Check cooldown
    user_id = message.from_user.id
    cooldown_key = f"cooldown:cmd_profile:{user_id}"
    
    ttl = await container.redis.ttl(cooldown_key)
    if ttl > 0:
        minutes = ttl // 60
        seconds = ttl % 60
        await message.answer(
            f"⏳ <b>Подожди немного!</b>\n"
            f"Команду можно использовать раз в 5 минут.\n"
    ...
    await container.redis.setex(cooldown_key, 300, "1")
...
@router.message(F.text.regexp(r"(?i)(?:^|\s)/senseitop(@\w+)?\b"))
