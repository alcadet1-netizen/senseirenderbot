# Check for cross mark in duel service file - safe version
...
print('Looking for string containing cross mark + "Это не вам."')
...
target_with_warning = b'\xe2\x9a\xa0 \xd0\x9f\xd0\xbe\xd1\x8a\xd0\xba\xd0\xb5\xd0\xben\xd0\xbb\xd0\xbe\x2e'
print('')
print('Looking for string containing warning + "Покенобо."')
...
# Let's look at the specific area where we saw issues before (around position 5131 from earlier)
print('')
print('=== Checking area around position 5131 ===')
start = max(0, 5131-50)
...
# Let's also check around position 8253 where we saw warning earlier
print('')
print('=== Checking area around position 8253 (warning) ===')
start = max(0, 8253-50)
end = min(len(data), 8253+50)
chunk = data[start:end]
print(f'Bytes from {start} to {end}:')
print(chunk.hex())
print()
